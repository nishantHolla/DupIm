...
int main(int argc, char *argv[]) {
  ...
  if (std::filesystem::is_directory(TARGET) == false) {
    io.output(SisIO::messageType::error, "Could not find target directory.");
    return 4;
  }
...
bool isImageFile(const std::filesystem::path &filePath) {
  if (std::filesystem::is_regular_file(filePath) == false) return false;
...
void compute(const std::filesystem::path &filePath,
  ...
  for (auto &entry : std::filesystem::recursive_directory_iterator(filePath)) {
    if (isImageFile(entry.path()) == false) continue;
    ...
    for (auto &pair : hashMap) {
      const int hammingDistance = ph_hamming_distance(pair.first, imageHash);
